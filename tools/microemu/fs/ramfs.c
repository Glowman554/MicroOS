...
void ramfs_write(vfs_mount_t* mount, file_t* file, void* buf, size_t size, size_t offset) {
	...
	if (new_file_size > node->fsize) {
		node->fdata = realloc(node->fdata, new_file_size);
...
void ramfs_truncate(vfs_mount_t* mount, file_t* file, size_t new_size) {
	ramfs_node_t* node = (ramfs_node_t*) file->driver_specific_data;
	node->fdata = realloc(node->fdata, new_size);
	node->fsize = new_size;
	file->size = node->fsize;
}

void ramfs_delete(vfs_mount_t* mount, file_t* file) {
    fatalf("ramfs_delete not implemented");
}
...
void ramfs_mkdir(vfs_mount_t* mount, char* path) {
	...
	for (int i = 0; i < sizeof(node->childs) / sizeof(node->childs[0]); i++) {
		if (!node->childs[i]) {
			ramfs_node_t* child = (ramfs_node_t*) malloc(sizeof(ramfs_node_t));
...
void ramfs_delete_dir(vfs_mount_t* mount, char* path) {
    fatalf("ramfs_delete_dir not implemented");
}


vfs_mount_t* get_ramfs(char* name) {
	int name_len = strlen(name);
	vfs_mount_t* mount = (vfs_mount_t*) malloc(sizeof(vfs_mount_t) + name_len + 1);
	...
	ramfs_node_t* root = (ramfs_node_t*) malloc(sizeof(ramfs_node_t));
