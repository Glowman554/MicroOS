...
cpu_registers_t* sys_ipv4_resolve_route(cpu_registers_t* regs) {
	if (regs->ebx >= num_nic_drivers || !nic_drivers[regs->ebx]) {
		abortf("sys_ipv4_resolve_route: invalid NIC driver index %d", regs->ebx);
	}

	network_stack_t* stack = (network_stack_t*) nic_drivers[regs->ebx]->driver.driver_specific_data;
	if (!stack || !regs->esi) {
		abortf("sys_ipv4_resolve_route: network stack not initialized for NIC driver index %d or invalid mac pointer", regs->ebx);
	}
