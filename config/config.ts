...
async function configurePragmaOnce(configMenu: ConfigMenu): Promise<void> {  
    ...
    for (let i = 0; i < configMenu.items.length; i++) {
        ...
        if (typeof(item.value) == "boolean") {
            console.log(`${i + 1}. ${item.name} [${item.value ? "Enabled" : "Disabled"}]`);
    ...
    if (isNaN(selectedOption) || selectedOption < 1 || selectedOption > configMenu.items.length) {
...
function generateConfiguration(configMenu: ConfigMenu): string {
    ...
    for (let i = 0; i < configMenu.items.length; i++) {
        ...
        if (typeof(configItem.value) == "boolean") {
            ...
            config += generateConfiguration(configItem.value as ConfigMenu);
        } else {
            config += `#define ${configItem.name} ${configItem.value}\n`;
        }
    }
...
async function main() {
    ...
    const configuration = JSON.parse(Deno.readTextFileSync(input)) as ConfigFile;

    
    let userConfiguration: ConfigMenu | undefined;
    try {
        if (!clean) {
            userConfiguration = JSON.parse(Deno.readTextFileSync(userConfigFile)) as ConfigMenu;
        }
    } catch (_) {}

    const configMenu: ConfigMenu = mergeDefault(configuration.configuration, userConfiguration) as ConfigMenu;
    ...
    Deno.writeTextFileSync(configuration.file, "#pragma once\n\n" + generateConfiguration(configMenu));

    if (!clean) {
        console.log(`Writing ${userConfigFile}...`);
        Deno.writeTextFileSync(userConfigFile, JSON.stringify(configMenu, null, "\t"));
    }
}
