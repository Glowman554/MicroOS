...
void error(char *fmt, ...) {
    char buffer[256]; // You can adjust the buffer size as needed
    va_list args;
    va_start(args, fmt);
...
Token *consume(char *op) {
	if (token->kind != TK_RESERVED || strlen(op) != token->len || strncmp(token->str, op, token->len)) return NULL;
	Token *t = token;
	token = token->next;
	return t;
}
...
Token *peek(char *s) {
	if (token->kind != TK_RESERVED || strlen(s) != token->len || strncmp(token->str, s, token->len)) return NULL;
	return token;
}
...
static char *starts_with_reserved(char *p) {
	// Keyword
	static char *kw[] = {"return", "if", "else", "while", "for", "int", "char", "sizeof", "struct", "typedef", "short", "long", "void", "_Bool", "enum", "static", "break", "continue", "goto", "switch", "case", "default", "extern", "_Alignof", "do", "signed"};
...
static Token *read_char_literal(Token *cur, char *start) {
	...
	if (*p != '\'') error_at(start, "char literal too long");
	p++;
...
static Token *read_int_literal(Token *cur, char *start) {
	...
	if (is_alnum(*p)) error_at(p, "invalid digit");
