...
bool command_received(char* command, bool* should_break, char* stdin) {
	*should_break = false;
	...
	if (cmd_type == NORMAL) { //Normal command, just run it and send the stdin
		...
		if (!found_command) {
			term_printf("Error: Command not found: '%s'\n", command);
		}
		...
		if (cmd_type == PIPE_FILE || cmd_type == PIPE_FILE_APPEND) { //If the command is a pipe to a file, we need to open the file and write the stdout to it
			...
			if (!canresolve) {
				term_printf("Error: No such file or directory to create output file in: %s\n", current_command);
				return false;
			}

			FILE* file = fopen(file_to_create, "w");
			if (file == NULL) {
				term_printf("Error: Failed to open file: %s\n", file_to_create);
				return false;
			}
		...
		term_printf("Error: Got an unknown command type, this shouldn't happen. %s\n", command);
	}
...
int spawn_process(char** argv, char** terminal_envp, pipe stdout, pipe stdin) {
	...
	if (strstr(executable, "/terminal")) {
		term_printf("Using legacy terminal. Setting VTERM to 0 for command!\n");
		export("export VTERM=0");
	}
	...
	bool terminal_ipc_ready = false;
	ipc_init_mapping(IPC_CONNECTION_VOUTPUT, pid);
	bool voutput_ipc_ready = false;
	
	set_env(SYS_ENV_PIN, (void*) 0);
	while (get_proc_info(pid)) {
		if (terminal_ipc_ready) {
			char out[0x1000] = { 0 };
			...
			if (ipc_init_host(IPC_CONNECTION_TERMINAL)) {
				terminal_ipc_ready = true;
			}
		}
		
		if (voutput_ipc_ready) {
			char out[0x1000] = { 0 };
			if (ipc_message_ready(IPC_CONNECTION_VOUTPUT, (void*) out)) {
				vconsole_puts(out);
				ipc_ok(IPC_CONNECTION_VOUTPUT);
			}
		} else {
			if (ipc_init_host(IPC_CONNECTION_VOUTPUT)) {
				voutput_ipc_ready = true;
			}
		}
