...
int main(int argc, char** argv, char** envp) {
    ...
    while (true) {
        ...
        if (prev_info.x != info.x || prev_info.y != info.y ) {
            should_redraw = true;
        }
        prev_info = info;
        ...
        for (int i = 0; i < window_get_count(); i++) {
            window_instance_t* w = window_get(i);
            if (w && w->is_external && w->update && !w->is_minimized) {
                event_t none_evt;
                none_evt.type = EVENT_NONE;
                none_evt.key = 0;
                none_evt.arrow = 0;
        ...
        desktop_poll_messages();

        unsigned long now_clock = (unsigned long)time(NULL);
        if (now_clock != last_clock_time) {
            last_clock_time = now_clock;
            should_redraw = true;
        }
