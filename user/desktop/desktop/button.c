...
void button_draw(button_t* btn, window_instance_t* w) {
    uint32_t bg = btn->is_hovered ? btn->hover_color : btn->bg_color;

    window_fill_rect(w, btn->x, btn->y, btn->width, btn->height, bg);

    int text_y = btn->y + (btn->height - 16) / 2;
    window_draw_string_bg(w, btn->x + 8, text_y, btn->label, btn->text_color, bg);
    
    window_draw_line(w, btn->x, btn->y, btn->x + btn->width, btn->y, btn->border_color);
    window_draw_line(w, btn->x, btn->y + btn->height - 1, btn->x + btn->width, btn->y + btn->height - 1, btn->border_color);
    window_draw_line(w, btn->x, btn->y, btn->x, btn->y + btn->height, btn->border_color);
    window_draw_line(w, btn->x + btn->width - 1, btn->y, btn->x + btn->width - 1, btn->y + btn->height, btn->border_color);
}
