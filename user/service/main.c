...
int wait_for_list_reply(service_list_reply_t* reply) {
...
int wait_for_op_reply(service_op_reply_t* reply) {
    for (int i = 0; i < WAIT_RETRIES; i++) {
...
void cmd_list(void) {
    ...
    for (int i = 0; i < reply.count; i++) {
        ...
        printf("\tcommand: %s\n", svc->command);
        printf("\tstatus: %s\n", status_str);
        if (svc->status != SERVICE_STATUS_STOPPED && svc->status != SERVICE_STATUS_FAILED) {
            printf("\tpid: %d\n", svc->pid);
            if (svc->retry > 0) {
...
int cmd_op(uint32_t topic, const char* name, const char* op_name) {
