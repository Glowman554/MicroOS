...
static VarScope *find_var(Token *tok) {
	for (VarScope *sc = var_scope; sc; sc = sc->next)
		if (strlen(sc->name) == tok->len && !strncmp(tok->str, sc->name, tok->len)) return sc;
	return NULL;
}
...
static Var *new_gvar(char *name, Type *ty, bool is_static, bool emit, bool _extern) {
	Var *var = new_var(name, ty, false);
	var->is_static = is_static;
	var->is_extern = _extern;
	push_scope(name)->var = var;
...
static Type *basetype(StorageClass *sclass) {
	...
	while (is_typename()) {
		...
		if (peek("typedef") || peek("static") || peek("extern")) {
			if (!sclass) error_tok(tok, "storage class specifier is not allowed");
			...
			if (*sclass & (*sclass - 1)) error_tok(tok, "typedef, static and extern may not be used together");
			continue;
		}
...
static Type *type_suffix(Type *ty) {
	...
	Token *tok = token;
	ty = type_suffix(ty);
	if (ty->is_incomplete) error_tok(tok, "incomplete element type");
...
static Type *struct_decl(void) {
	...
	if (!consume("{")) return struct_type();

	Type *ty;

	TagScope *sc = NULL;
	if (tag) sc = find_tag(tag);
	...
	for (Member *mem = ty->members; mem; mem = mem->next) {
		if (mem->ty->is_incomplete) error_tok(mem->tok, "incomplete struct member");
...
static Function *function(void) {
	...
	new_gvar(name, func_type(ty), false, false, false);
...
static Initializer *new_init_zero(Initializer *cur, int nbytes) {
	for (int i = 0; i < nbytes; i++) cur = new_init_val(cur, 1, 0);
	return cur;
}
...
static Initializer *gvar_init_string(char *p, int len) {
	...
	for (int i = 0; i < len; i++) cur = new_init_val(cur, 1, p[i]);
	return head.next;
}
...
static Initializer *gvar_initializer2(Initializer *cur, Type *ty) {
	...
	if (ty->kind == TY_STRUCT) {
		...
		if (mem) cur = new_init_zero(cur, ty->size - mem->offset);
		return cur;
	}
...
static void global_var(void) {
	...
	Var *var = new_gvar(name, ty, sclass == STATIC, true, sclass == EXTERN);
...
static Node *declaration(void) {
	...
	StorageClass sclass;
	Type *ty = basetype(&sclass);
	if ((tok = consume(";"))) return new_node(ND_NULL, tok);
	...
	if (sclass == STATIC) {
		...
		Var *var = new_gvar(new_label(), ty, true, true, false);
		push_scope(name)->var = var;
...
static bool is_typename(void) { return peek("void") || peek("_Bool") || peek("char") || peek("short") || peek("int") || peek("long") || peek("enum") || peek("struct") || peek("typedef") || peek("static") || peek("extern") || peek("signed") || find_typedef(token); }
...
static long eval2(Node *node, Var **var) {
	switch (node->kind) {
		...
		case ND_NUM:
			return node->val;
		case ND_ADDR:
			if (!var || *var || node->lhs->kind != ND_VAR || node->lhs->var->is_local) error_tok(node->tok, "invalid initializer");
			*var = node->lhs->var;
			return 0;
		case ND_VAR:
...
static Node *assign(void) {
	...
	if ((tok = consume("="))) return new_binary(ND_ASSIGN, node, assign(), tok);

	if ((tok = consume("*="))) return new_binary(ND_MUL_EQ, node, assign(), tok);

	if ((tok = consume("/="))) return new_binary(ND_DIV_EQ, node, assign(), tok);

	if ((tok = consume("%="))) return new_binary(ND_MOD_EQ, node, assign(), tok);

	if ((tok = consume("<<="))) return new_binary(ND_SHL_EQ, node, assign(), tok);

	if ((tok = consume(">>="))) return new_binary(ND_SHR_EQ, node, assign(), tok);

	if ((tok = consume("&="))) return new_binary(ND_BITAND_EQ, node, assign(), tok);

	if ((tok = consume("|="))) return new_binary(ND_BITOR_EQ, node, assign(), tok);

	if ((tok = consume("^="))) return new_binary(ND_BITXOR_EQ, node, assign(), tok);
...
static Node *logor(void) {
	Node *node = logand();
	Token *tok;
	while ((tok = consume("||"))) node = new_binary(ND_LOGOR, node, logand(), tok);
	return node;
}
...
static Node *logand(void) {
	Node *node = bitor ();
	Token *tok;
	while ((tok = consume("&&"))) node = new_binary(ND_LOGAND, node, bitor (), tok);
	return node;
}
...
static Node * bitor (void) {
	Node *node = bitxor();
	Token *tok;
	while ((tok = consume("|"))) node = new_binary(ND_BITOR, node, bitxor(), tok);
	return node;
}
...
static Node *bitxor(void) {
	Node *node = bitand();
	Token *tok;
	while ((tok = consume("^"))) node = new_binary(ND_BITXOR, node, bitxor(), tok);
	return node;
}
...
static Node *bitand(void) {
	Node *node = equality();
	Token *tok;
	while ((tok = consume("&"))) node = new_binary(ND_BITAND, node, equality(), tok);
	return node;
}
...
static Node *new_add(Node *lhs, Node *rhs, Token *tok) {
	...
	if (lhs->ty->base && is_integer(rhs->ty)) return new_binary(ND_PTR_ADD, lhs, rhs, tok);
	if (is_integer(lhs->ty) && rhs->ty->base) return new_binary(ND_PTR_ADD, rhs, lhs, tok);
...
static Node *new_sub(Node *lhs, Node *rhs, Token *tok) {
	...
	if (lhs->ty->base && is_integer(rhs->ty)) return new_binary(ND_PTR_SUB, lhs, rhs, tok);
	if (lhs->ty->base && rhs->ty->base) return new_binary(ND_PTR_DIFF, lhs, rhs, tok);
	error_tok(tok, "invalid operands");
}
...
static Node *unary(void) {
	...
	if ((tok = consume("-"))) return new_binary(ND_SUB, new_num(0, tok), cast(), tok);
	if ((tok = consume("&"))) return new_unary(ND_ADDR, cast(), tok);
	if ((tok = consume("*"))) return new_unary(ND_DEREF, cast(), tok);
	if ((tok = consume("!"))) return new_unary(ND_NOT, cast(), tok);
	if ((tok = consume("~"))) return new_unary(ND_BITNOT, cast(), tok);
	if ((tok = consume("++"))) return new_unary(ND_PRE_INC, unary(), tok);
	if ((tok = consume("--"))) return new_unary(ND_PRE_DEC, unary(), tok);
	return postfix();
}
...
static Node *stmt_expr(Token *tok) {
	...
	if (cur->kind != ND_EXPR_STMT) error_tok(cur->tok, "stmt expr returning void is not supported");
...
static Node *primary(void) {
	...
	if ((tok = consume("sizeof"))) {
		...
		Node *node = unary();
		add_type(node);
		if (node->ty->is_incomplete) error_tok(node->tok, "incomplete type");
	...
	if ((tok = consume_ident())) {
		...
		if (consume("(")) {
			...
			VarScope *sc = find_var(tok);
			if (sc) {
				if (!sc->var || sc->var->ty->kind != TY_FUNC) error_tok(tok, "not a function");
				node->ty = sc->var->ty->return_ty;
			} else if (!strcmp(node->funcname, "__builtin_va_start")) {
				node->ty = void_type;
			} else if (!strcmp(node->funcname, "_syscall")) {
				node->ty = void_type;
			} else {
